...
export const FinancialStatements: React.FC<{ type: StatementType }> = ({ type }) => {
    ...
    return (
      <div className="bg-white rounded-xl border border-slate-200 shadow-sm flex-1 flex flex-col h-full overflow-hidden">
        <div className="p-4 border-b border-slate-200 bg-slate-50 shrink-0 flex justify-between items-center">
          <div>
            <h3 className="text-lg font-black text-[#1D3557]">Trial Balance</h3>
        ...
        <div className="flex-1 overflow-auto">
          {loading ? <div className="flex items-center justify-center h-full text-slate-400 text-sm gap-2"><div className="animate-spin w-5 h-5 border-2 border-blue-500 border-t-transparent rounded-full"/><span>Generating Trial Balance...</span></div> : (
            <table className="w-full text-sm border-collapse">
              <thead className="bg-[#E4ECEF] sticky top-0 text-xs text-slate-700 uppercase font-black border-b border-slate-300 shadow-sm z-10">
                <tr>
        ...
        {!loading && rows.length > 0 && (
          <div className={`flex font-black text-sm shrink-0 ${isBalanced ? 'bg-[#1D3557] text-white' : 'bg-red-700 text-white'}`}>
  ...
  const TreeNode: React.FC<{ item: any; depth?: number }> = ({ item, depth = 0 }) => {
    ...
    return (
      <div>
        <div
          className={`flex items-center justify-between p-1.5 cursor-pointer border-b border-slate-100 hover:bg-blue-50/50 ${depth === 0 ? 'bg-slate-50 font-black text-[#1D3557]' : depth === 1 ? 'font-bold text-slate-700' : 'font-medium text-slate-600 text-sm'}`}
  ...
  return (
    <div className="bg-white rounded-xl border border-slate-200 shadow-sm flex-1 flex flex-col h-full overflow-hidden">
      <div className="p-4 border-b border-slate-200 bg-slate-50 shrink-0 flex justify-between items-center">
        <div>
          <h3 className="text-lg font-black text-[#1D3557]">{titles[type]}</h3>
      ...
      <div className="flex-1 overflow-auto flex text-sm">
        {loading ? (
          <div className="flex items-center justify-center w-full text-slate-400 gap-2"><div className="animate-spin w-5 h-5 border-2 border-blue-500 border-t-transparent rounded-full"/><span>Generating {titles[type]}...</span></div>
        ) : data ? (
          <>
            {/* Left Panel */}
            <div className="flex-1 border-r border-slate-300 flex flex-col">
              <div className="bg-[#E4ECEF] px-4 py-2 font-black text-slate-700 border-b border-slate-300 text-xs uppercase tracking-widest flex justify-between shrink-0">
              ...
              <div className="flex-1 overflow-auto">
                ...
                {type === 'PROFIT_LOSS' && data.netProfitAfterTax > 0 && (
                  <div className="flex justify-between p-2 px-3 border-t-2 border-green-500 bg-green-50 font-black text-green-800">
                    <span>Net Profit (Transferred)</span>
                    <span>₹{data.netProfitAfterTax.toLocaleString(undefined,{minimumFractionDigits:2})}</span>
                  </div>
                )}
              </div>
              <div className="bg-[#1D3557] text-white px-4 py-2 font-black flex justify-between text-sm shrink-0">
                <span>Total</span>
            ...
            <div className="flex-1 flex flex-col">
              ...
              <div className="flex-1 overflow-auto">
                ...
                {type === 'PROFIT_LOSS' && data.netProfitAfterTax < 0 && (
                  <div className="flex justify-between p-2 px-3 border-t-2 border-red-500 bg-red-50 font-black text-red-800">
