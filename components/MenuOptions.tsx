...
const MenuOptions: React.FC<MenuOptionsProps> = ({
  ...
  return (
    <div className="relative" ref={menuRef}>
      <button
        onClick={toggleMenu}
        className="p-2 rounded-xl hover:bg-white/50 text-slate-600 hover:text-blue-600 transition-all shadow-sm border border-transparent hover:border-slate-200"
            ...
            {menuItems.map((item) => {
              ...
              return (
                <button
                  ...
                  className={`w-full flex items-center gap-3 px-4 py-3 text-left transition-colors ${item.id === 'logout' ? 'hover:bg-red-50 text-red-700' : 'hover:bg-slate-50 text-slate-700'}`}
                >
                  <item.icon size={18} className={item.id === 'logout' ? 'text-red-500' : 'text-slate-500'} />
