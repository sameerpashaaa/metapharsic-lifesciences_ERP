...
const MultiBranchDashboard: React.FC = () => {
  ...
  if (loading) return (
    <div className="p-20 text-center">
      <RefreshCcw className="mx-auto text-blue-600 animate-spin mb-4" size={48}/>
  ...
  return (
    <div className="p-6 space-y-6 bg-slate-50 min-h-full">
      ...
      <div className="flex justify-between items-end mb-4">
        <div>
          <h2 className="text-2xl font-black text-[#1D3557] tracking-tight flex items-center gap-3">
            <Globe className="text-blue-600" size={28}/> Enterprise Inventory Hub
          </h2>
          <p className="text-sm text-slate-500 font-medium">Consolidated monitoring across {branches.length} branches</p>
        </div>
        <div className="flex gap-3">
           <button className="bg-white border border-slate-200 px-4 py-2 rounded-lg text-xs font-black text-slate-600 shadow-sm hover:bg-slate-50 transition-all flex items-center gap-2">
              <ArrowRightLeft size={14}/> NEW TRANSFER
           </button>
           <button className="bg-[#1D3557] text-white px-4 py-2 rounded-lg text-xs font-black shadow-lg hover:shadow-blue-900/20 transition-all">
              GLOBAL SYNC
           </button>
        </div>
      </div>
      ...
      <div className="grid grid-cols-4 gap-4">
         {[
           { label: 'Total Branches', value: summary.totalBranches, icon: MapPin, color: 'text-blue-600', bg: 'bg-blue-50' },
           ...
           <div key={i} className="bg-white p-5 rounded-2xl border border-slate-200 shadow-sm flex items-center gap-4 hover:shadow-md transition-shadow cursor-default">
      ...
      <div className="grid grid-cols-3 gap-6">
         ...
         <div className="col-span-2 bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden flex flex-col">
            <div className="px-6 py-4 border-b border-slate-100 flex justify-between items-center">
               <h3 className="font-black text-slate-800 text-sm uppercase tracking-tighter">Branch-wise Inventory Distribution</h3>
               <span className="text-[10px] font-bold text-blue-600 bg-blue-50 px-2 py-1 rounded">LIVE FEED</span>
            </div>
                  ...
                  <tbody className="divide-y divide-slate-50">
                     {branches.map(b => (
                       <tr key={b.id} className="hover:bg-slate-50 transition-colors group">
                          ...
                          <td className="px-6 py-4 text-right font-black text-blue-700">{(Math.random() * 5000).toFixed(0)}</td>
                          <td className="px-6 py-4 text-right font-bold text-amber-600">{(Math.random() * 200).toFixed(0)}</td>
                          <td className="px-6 py-4 text-center">
                             <span className="px-2 py-0.5 rounded-full bg-green-100 text-green-700 text-[9px] font-black uppercase">Active</span>
                          </td>
                       </tr>
                     ))}
                  </tbody>
         ...
         <div className="col-span-1 bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden flex flex-col">
            <div className="px-6 py-4 border-b border-slate-100 bg-red-50/30 flex justify-between items-center">
               <h3 className="font-black text-red-800 text-sm uppercase tracking-tighter">Stock Alarms</h3>
            ...
            <div className="flex-1 overflow-auto p-4 space-y-3">
               ...
               <button className="w-full py-2 text-[10px] font-black text-slate-400 uppercase tracking-widest hover:text-slate-600 transition-colors">VIEW FULL ENGINE report</button>
            </div>
         </div>
      </div>
    </div>
