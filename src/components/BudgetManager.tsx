      ...
      <div className="bg-white rounded-xl border border-slate-200 shadow-sm flex-1 flex flex-col h-full overflow-hidden">
        ...
        <div className="flex-1 overflow-auto p-6 bg-slate-50">
          ...
          <div className="max-w-3xl mx-auto bg-white border border-slate-200 shadow-sm rounded p-6">
            <h4 className="text-xs font-black text-slate-400 uppercase tracking-widest mb-6 border-b pb-2">Budget Header</h4>
            <div className="grid grid-cols-3 gap-4 mb-6">
              <div className="col-span-3">
                <label className="block text-[10px] font-black text-slate-500 uppercase mb-1">Budget Name *</label>
                <input value={form.budgetName} onChange={e => setForm({...form, budgetName: e.target.value})} className="w-full text-sm font-bold border-b border-slate-300 p-1 outline-none focus:border-blue-500 bg-yellow-50" placeholder="e.g. FY 2025-26 Operations Budget"/>
              </div>
              <div>
                <label className="block text-[10px] font-black text-slate-500 uppercase mb-1">Financial Year</label>
              ...
              <div>
                <label className="block text-[10px] font-black text-slate-500 uppercase mb-1">Budget Type</label>
              ...
              <div>
                <label className="block text-[10px] font-black text-slate-500 uppercase mb-1">Period</label>
              ...
              <div>
                <label className="block text-[10px] font-black text-slate-500 uppercase mb-1">Notes</label>
            ...
            <h4 className="text-xs font-black text-slate-400 uppercase tracking-widest mb-4 border-b pb-2">Account Allocations</h4>
            ...
            <button onClick={() => setForm({...form, allocations:[...form.allocations,{costCenter:'',account:'',budgetAmount:0,description:''}]})} className="mt-2 text-xs font-bold text-blue-600 hover:underline flex items-center gap-1"><Plus size={12}/> Add Row</button>
            <div className="mt-4 p-3 bg-blue-50 rounded flex justify-between text-sm font-black text-[#1D3557]">
  ...
  return (
    <div className="bg-white rounded-xl border border-slate-200 shadow-sm flex-1 flex flex-col h-full overflow-hidden">
      <div className="p-4 border-b border-slate-200 bg-slate-50 shrink-0 flex justify-between items-center">
        <div>
          <h3 className="text-lg font-black text-slate-800">Budget vs Actuals</h3>
      ...
      <div className="flex-1 overflow-auto p-4 bg-slate-100">
        {loading ? (
          <div className="flex items-center justify-center h-full gap-2 text-slate-400 text-sm"><div className="animate-spin w-5 h-5 border-2 border-blue-500 border-t-transparent rounded-full"/>Loading budgets...</div>
        ) : budgets.map(b => {
          ...
          return (
            <div key={b.id} className="bg-white border border-slate-200 shadow-sm rounded mb-4 overflow-hidden">
              <div className="p-4 flex justify-between items-start border-b border-slate-100">
                <div>
                  <div className="font-black text-[#1D3557] text-base">{b.budgetName}</div>
                  <div className="text-xs text-slate-500 font-medium mt-0.5">{b.budgetType} • {b.financialYear} • {b.startDate} to {b.endDate}</div>
                </div>
                <div className="flex items-center gap-3">
                  <span className={`text-xs font-black px-2 py-0.5 rounded-full border ${diff >= 0 ? 'bg-green-100 text-green-700 border-green-200' : 'bg-red-100 text-red-700 border-red-200'}`}>
                    {diff >= 0 ? 'Under Budget' : 'Over Budget'}
                  </span>
                  <button className="text-xs font-bold text-blue-600 hover:underline flex items-center gap-1"><BarChart2 size={12}/> Drill Down</button>
                </div>
              </div>
              <div className="p-4 grid grid-cols-4 gap-4">
                <div>
                  <div className="text-[10px] text-slate-500 uppercase font-bold">Total Budget</div>
                  <div className="font-black text-slate-800 text-lg">₹{(b.totalBudget || 0).toLocaleString()}</div>
                </div>
                <div>
                  <div className="text-[10px] text-slate-500 uppercase font-bold">Actual Spent</div>
                  <div className="font-black text-slate-800 text-lg">₹{(b.totalActual || 0).toLocaleString()}</div>
                </div>
                <div>
                  <div className="text-[10px] text-slate-500 uppercase font-bold">Variance</div>
                  <div className={`font-black text-lg flex items-center gap-1 ${diff >= 0 ? 'text-green-600' : 'text-red-600'}`}>
                ...
                <div>
                  ...
                  <div className="font-black text-slate-800 text-lg">{usedPct.toFixed(1)}%</div>
