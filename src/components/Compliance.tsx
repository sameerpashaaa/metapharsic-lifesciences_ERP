    ...
    >
      ...
      <div className="mt-6">
        {activeTab === 'LICENSES' && (
          <div className="space-y-4">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
              <div>
                <h3 className="text-sm font-black text-slate-700 uppercase tracking-widest">License Register</h3>
              ...
              <div className="flex bg-slate-100 p-1 rounded-xl border border-slate-200 w-fit">
                <button
                  ...
                  className={`p-2 rounded-lg transition-all ${licenseViewMode === 'grid' ? 'bg-white text-blue-700 shadow-sm' : 'text-slate-500 hover:text-slate-800'}`}
                >
                  <Grid3X3 size={18} />
                </button>
                <button
                  ...
                  className={`p-2 rounded-lg transition-all ${licenseViewMode === 'list' ? 'bg-white text-blue-700 shadow-sm' : 'text-slate-500 hover:text-slate-800'}`}
                >
                  <List size={18} />
                </button>
              </div>
            ...
            {licenseViewMode === 'grid' ? (
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {licensesData?.map(license => (
                  <div key={license.id} className="bg-white p-6 rounded-xl border border-slate-200 shadow-sm relative overflow-hidden group hover:shadow-md transition-shadow">
                    ...
                    <div className="relative z-10">
                      ...
                      <div className="flex items-center gap-2 mt-5">
                        <button onClick={() => handleEditLicense(license)} className="flex-1 px-3 py-2 rounded-lg bg-blue-50 text-blue-700 text-xs font-black uppercase tracking-wider hover:bg-blue-100 flex items-center justify-center gap-2">
                          <Edit size={14} /> Edit
                        </button>
                        <button onClick={() => handleDeleteLicense(license)} className="px-3 py-2 rounded-lg bg-red-50 text-red-700 text-xs font-black uppercase tracking-wider hover:bg-red-100" title="Delete license">
                          <Trash2 size={14} />
                        </button>
                      </div>
                    </div>
              ...
              <DataTable
                columns={[
                  ...
                  { key: 'expiry_date', label: 'Expires', width: '14%', render: (v) => v ? new Date(v).toLocaleDateString() : '--' },
                  {
                    key: 'actions',
                    label: 'Actions',
                    width: '14%',
                    render: (_v, row) => (
                      <div className="flex items-center gap-2">
                        <button onClick={() => handleEditLicense(row)} className="p-2 rounded-lg text-blue-600 hover:bg-blue-50" title="Edit license">
                          <Edit size={16} />
                        </button>
        ...
        {activeTab === 'H1_REGISTER' && (
          <DataTable 
            columns={[
              { key: 'entry_date', label: 'Date', width: '15%', render: (v) => new Date(v).toLocaleDateString() },
        ...
        {activeTab === 'COLD_CHAIN' && (
          <DataTable 
            columns={[
              { key: 'log_date', label: 'Date', width: '20%', render: (v) => new Date(v).toLocaleDateString() },
        ...
        {activeTab === 'AUDITS' && (
          <DataTable 
            columns={[
              { key: 'audit_date', label: 'Date', width: '18%', render: (v) => new Date(v).toLocaleDateString() },
        ...
        >
          <form onSubmit={handleSaveRecord} className="space-y-5">
            ...
            <div className="flex justify-end gap-3 pt-4 border-t border-slate-100">
              ...
              <button type="submit" disabled={saving} className="px-5 py-2.5 rounded-xl bg-blue-600 text-white text-sm font-bold hover:bg-blue-700 disabled:opacity-60 flex items-center gap-2">
...
const fieldClass = "w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl text-sm font-medium focus:outline-none focus:ring-4 focus:ring-blue-500/10 focus:border-blue-500";
