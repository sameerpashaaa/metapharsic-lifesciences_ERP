...
import { Save, Search, Plus, Archive, Calculator, TrendingUp, Download, Printer, RefreshCw, AlertCircle, Edit3, Globe, Info, Trash2 } from 'lucide-react';
...
export const ItemMaster: React.FC = () => {
 ...
 if (!window.confirm('⚠️ Are you sure you want to delete this product? This action will hide the item from the master list and inventory, though historical data will be preserved.')) return;
 try {
 await apiClient.delete(`/api/inventory/${id}`);
 await refetch();
 setSuccessMsg('SKU deleted from Item Master');
 setTimeout(() => setSuccessMsg(''), 3000);
 } catch (err: any) {
 console.error('Delete failed:', err);
 alert('Failed to delete item: ' + (err.data?.error || err.message || 'Unknown error'));
 }
 };
 ...
 <button onClick={() => { setView('LIST'); setEditingId(null); setForm(defaultForm); }} className="text-slate-300 hover:text-white uppercase text-[10px] font-bold tracking-widest bg-white/10 px-3 py-1 rounded transition-colors">Close (Esc)</button>
 </div>
 ...
 className={`py-3 text-[10px] font-bold uppercase border-b-2 transition-all tracking-widest whitespace-nowrap ${activeTab === t.id ? 'border-accent text-accent' : 'border-transparent text-slate-400 hover:text-slate-600'}`}
 >
 {t.label}
 </button>
 ))}
 ...
 <label className="block text-[10px] font-bold text-slate-500 uppercase mb-1.5 tracking-widest">Item Code *</label>
 <input value={form.itemCode} onChange={e => setForm({...form, itemCode: e.target.value})} className="w-full p-2.5 bg-slate-50 border border-slate-200 rounded-xl text-sm font-bold focus:ring-2 focus:ring-blue-500/20 focus:border-accent outline-none transition-all" placeholder="e.g. ITM-765"/>
 </div>
 <div className="col-span-3">
 <label className="block text-[10px] font-bold text-slate-500 uppercase mb-1.5 tracking-widest">Product Name *</label>
 <input value={form.itemName} onChange={e => setForm({...form, itemName: e.target.value})} className="w-full p-2.5 bg-slate-50 border border-slate-200 rounded-xl text-sm font-bold focus:ring-2 focus:ring-blue-500/20 focus:border-accent outline-none transition-all" placeholder="Brand Name"/>
 </div>
 </div>

 <div className="bg-white p-6 rounded-xl border border-slate-200 shadow-sm grid grid-cols-1 md:grid-cols-2 gap-6">
 <div>
 <label className="block text-[10px] font-bold text-slate-500 uppercase mb-1.5 tracking-widest">Generic Name (Salt) *</label>
 <input value={form.genericName} onChange={e => setForm({...form, genericName: e.target.value})} className="w-full p-2.5 bg-slate-50 border border-slate-200 rounded-xl text-sm font-bold focus:ring-2 focus:ring-blue-500/20 focus:border-accent outline-none transition-all" placeholder="Generic salt composition"/>
 </div>
 <div>
 <label className="block text-[10px] font-bold text-slate-500 uppercase mb-1.5 tracking-widest">Manufacturer / Company *</label>
 <input value={form.manufacturer} onChange={e => setForm({...form, manufacturer: e.target.value})} className="w-full p-2.5 bg-slate-50 border border-slate-200 rounded-xl text-sm font-bold focus:ring-2 focus:ring-blue-500/20 focus:border-accent outline-none transition-all" placeholder="Manufacturing company"/>
 </div>
 </div>
 
 <div className="bg-white p-6 rounded-xl border border-slate-200 shadow-sm grid grid-cols-1 md:grid-cols-4 gap-6">
 <div>
 <label className="block text-[10px] font-bold text-slate-500 uppercase mb-1.5 tracking-widest">Stock Group</label>
 ...
 <div>
 <label className="block text-[10px] font-bold text-slate-500 uppercase mb-1.5 tracking-widest">Category</label>
 ...
 <div>
 <label className="block text-[10px] font-bold text-slate-500 uppercase mb-1.5 tracking-widest">Unit of Measure</label>
 ...
 <span className="text-[10px] font-bold text-slate-500 uppercase tracking-widest">Is Active</span>
 </label>
 </div>
 </div>
 </div>
 ...
 <label className="block text-[10px] font-bold text-slate-400 uppercase tracking-widest">Standard Purchase Rate</label>
 ...
 className="w-full text-3xl font-bold pl-5 pr-2 py-2 border-b-2 border-slate-100 focus:border-accent outline-none transition-all placeholder-slate-200" 
 placeholder="0.00"
 />
 </div>
 ...
 <div className="bg-white p-6 rounded-xl border border-slate-200 shadow-sm space-y-4">
 <div className="flex justify-between items-center">
 <label className="block text-[10px] font-bold text-slate-400 uppercase tracking-widest">Standard Selling Rate</label>
 ...
 <div>
 <label className="block text-[10px] font-bold text-slate-500 uppercase mb-1 tracking-widest">MRP (Max Retail)</label>
 ...
 <div>
 <label className="block text-[10px] font-bold text-slate-500 uppercase mb-1 tracking-widest">PTR (Retailer Rate)</label>
 ...
 <div>
 <label className="block text-[10px] font-bold text-slate-500 uppercase mb-1 tracking-widest">PTS (Stockist Rate)</label>
 ...
 {activeTab === 'STATUTORY' && (
 <div className="bg-white p-8 rounded-xl border border-slate-200 shadow-sm space-y-8 animate-fadeIn">
 <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
 <div>
 <label className="block text-[10px] font-bold text-slate-500 uppercase mb-2 tracking-widest">HSN / SAC Code</label>
 <input value={form.hsnCode} onChange={e => setForm({...form, hsnCode: e.target.value})} className="w-full p-3 border rounded-xl text-lg font-bold tracking-widest outline-none blur-none transition-all" placeholder="3004xxxx"/>
 </div>
 <div>
 <label className="block text-[10px] font-bold text-slate-500 uppercase mb-2 tracking-widest">GST Rate (%)</label>
 <div className="flex gap-2">
 {GST_RATES.map(rate => (
 <button key={rate} onClick={() => setForm({...form, taxRate: rate})} className={`flex-1 py-3 rounded-xl border font-bold text-sm transition-all ${form.taxRate === rate ? 'bg-accent text-white border-accent shadow-none' : 'bg-white text-slate-600 hover:bg-slate-50'}`}>
 {rate}%
 </button>
 ))}
 </div>
 ...
 {activeTab === 'INVENTORY' && (
 <div className="bg-white p-6 rounded-xl border border-slate-200 shadow-sm space-y-8 animate-fadeIn">
 <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
 <div className="space-y-4">
 <label className="block text-[10px] font-bold text-slate-500 uppercase tracking-widest">Opening Enterprise Stock</label>
 <div className="flex items-center gap-4">
 <input type="number" 
 value={form.openingStock || ''} 
 onChange={e => setForm({...form, openingStock: e.target.value === '' ? 0 : parseFloat(e.target.value)})} 
 className="flex-1 p-4 bg-slate-50 border-2 border-slate-100 rounded-xl text-2xl font-bold outline-none focus:border-accent transition-all" 
 placeholder="0"
 />
 ...
 <div>
 <label className="block text-[10px] font-bold text-slate-500 uppercase mb-2 tracking-widest">Reorder Level</label>
 <input type="number" 
 ...
 <div>
 <label className="block text-[10px] font-bold text-slate-500 uppercase mb-2 tracking-widest">Min Stock Limit</label>
 <input type="number" 
 ...
 {activeTab === 'SCHEMES' && (
 <div className="bg-white p-8 rounded-xl border border-slate-200 shadow-sm space-y-6 animate-fadeIn">
 <div className="flex items-center gap-4 text-accent mb-2">
 <Calculator size={24}/>
 <h4 className="font-bold text-lg uppercase tracking-tight">Promotional configuration</h4>
 </div>
 <div>
 <label className="block text-[10px] font-bold text-slate-500 uppercase mb-2 tracking-widest">Standard Offer / Scheme</label>
 <input value={form.scheme} onChange={e => setForm({...form, scheme: e.target.value})} className="w-full p-4 bg-slate-50 border border-slate-200 rounded-xl text-xl font-bold placeholder-slate-300 outline-none" placeholder="e.g. 10+1 or 5% DISCOUNT"/>
 ...
 <div>
 <h4 className="font-bold text-sm uppercase tracking-tight">Enterprise Inventory distribution</h4>
 <p className="text-[10px] text-blue-200 font-bold uppercase tracking-widest">Real-time cross-branch visibility</p>
 </div>
 <button className="bg-accent hover:bg-accent text-white p-2 rounded-lg transition-all shadow-none"><RefreshCw size={14}/></button>
 </div>
 ...
 <input 
 type="number" 
 className="w-24 p-2 text-right border rounded-lg font-bold text-accent outline-none focus:border-accent bg-white"
 value={branch.stock || ''}
 placeholder="0"
 onChange={e => {
 ...
 branch.stock > 0 ? 'bg-blue-100 text-accent' : 'bg-red-100 text-red-700'
 }`}>
 ...
 <div className="bg-white p-4 border-t border-slate-200 flex flex-wrap justify-between items-center shadow-none z-20 shrink-0 gap-4">
 <div className="flex gap-4">
 <div className="text-center px-4 border-r">
 <div className="text-[9px] font-bold text-slate-400 uppercase tracking-tighter">Landing Cost</div>
 <div className="text-xs font-bold text-slate-800">₹{form.landingCost}</div>
 </div>
 <div className="text-center px-4">
 <div className="text-[9px] font-bold text-slate-400 uppercase tracking-tighter">Profit Margin</div>
 <div className={`text-xs font-bold ${form.marginPercent > 0 ? 'text-green-600' : 'text-slate-400'}`}>{form.marginPercent}%</div>
 </div>
 </div>
 <div className="flex gap-3">
 <button onClick={() => { setView('LIST'); setEditingId(null); setForm(defaultForm); }} className="px-8 py-2 text-xs font-bold text-slate-500 uppercase tracking-widest hover:bg-slate-50 rounded-xl transition-all border border-slate-200">Discard Changes</button>
 <button onClick={handleSave} disabled={saving} className="px-8 py-2 text-xs font-bold text-white uppercase tracking-widest bg-accent rounded-xl hover:shadow-none hover:shadow-none active:scale-95 transition-all flex items-center gap-2 group disabled:opacity-50">
 ...
 <h3 className="text-xl font-bold text-[#1D3557] uppercase tracking-tight">Enterprise Asset Valuation</h3>
 <button 
 onClick={() => refetch()}
 className="flex items-center gap-2 px-3 py-1.5 bg-blue-50 text-accent rounded-lg text-[10px] font-bold uppercase tracking-widest hover:bg-blue-100 transition-all border border-blue-200 shadow-sm"
 >
 ...
 <span className="text-2xl font-bold text-accent">₹{(valuation.totalAssetValue / 100000).toFixed(2)}L</span>
 </div>
 <div className="flex justify-between items-center p-4 bg-slate-50 rounded-xl border border-slate-100">
 <span className="text-sm font-bold text-slate-500 uppercase tracking-widest">Market Value (MRP)</span>
 <span className="text-2xl font-bold text-emerald-700">₹{(valuation.totalMarketValue / 100000).toFixed(2)}L</span>
 </div>
 ...
 Last Database Sync: <span className="text-slate-600 font-bold">{new Date().toLocaleTimeString()}</span>
 </p>
 </div>
 </div> </div>
 </div>
 <div className="bg-[#1D3557] p-8 rounded-xl text-white relative overflow-hidden flex flex-col justify-center">
 ...
 <div className="flex gap-4">
 <div className="px-4 py-2 bg-white/10 rounded-lg text-xs font-bold uppercase tracking-widest border border-white/20">Sync Active</div>
 <div className="px-4 py-2 bg-white/10 rounded-lg text-xs font-bold uppercase tracking-widest border border-white/20">Audit Ready</div>
 </div>
 </div>
 ...
 <div className="w-8 h-8 rounded-lg bg-slate-100 border border-slate-200 flex items-center justify-center text-accent shrink-0 shadow-sm">
 <Archive size={16}/>
 </div>
 <div className="min-w-0">
 <div className="font-bold text-[#1D3557] truncate leading-tight uppercase text-xs">{row.name}</div>
 ...
 <span className={`text-[7px] font-bold tracking-widest px-1.5 py-0.5 rounded ${status === 'CRITICAL' ? 'bg-red-50 text-red-600 border border-red-100' : 'bg-slate-50 text-slate-400 border border-slate-100'}`}>
 {status}
 </span>
 </div>
 </div>
 ...
 <span className="text-[10px] font-bold text-slate-700">{row.hsnCode || '---'}</span>
 </div>
 <Badge 
 ...
 <div className={`p-1.5 rounded-lg border shadow-sm transition-all ${row.maintainBatches ? 'bg-blue-50 border-blue-100 text-accent' : 'bg-slate-50 border-slate-100 text-slate-300'}`} title="Batch Tracking">
 <Archive size={12}/>
 </div>
 ...
 <button onClick={() => handleEdit(row)} className="p-2 text-accent hover:bg-blue-50 rounded-lg transition-all hover:scale-110 active:scale-90" title="Edit Master Record">
 <Edit3 size={16}/>
 </button>
 <button onClick={(e) => { e.stopPropagation(); handleDelete(row.id); }} className="p-2 text-slate-400 hover:text-red-600 rounded-lg transition-all hover:scale-110 active:scale-90" title="Delete Master Record">
 <Trash2 size={16}/>
 </button>
 <button className="p-2 text-slate-400 hover:text-accent rounded-lg transition-all" title="View Detailed Analytics">
 <TrendingUp size={16}/>
 </button>
 </div>
 ) }
 ...
 Showing <span className="font-bold text-slate-800">{pagination.paginatedData.length}</span> of <span className="font-bold text-slate-800">{filteredData.length}</span> records
 </span>
 <div className="flex gap-2">
 <button
 ...
 <div className="px-6 py-2.5 bg-slate-100 text-[#1D3557] font-bold rounded-xl">
