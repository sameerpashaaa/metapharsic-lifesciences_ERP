        ...
        <div className="flex-1 overflow-auto p-6 bg-slate-50">
          <div className="max-w-3xl mx-auto bg-white border border-slate-200 shadow-sm p-6 rounded">
            <div className="grid grid-cols-3 gap-4 mb-6 border-b pb-4">
              <div><div className="text-[10px] text-slate-500 uppercase font-bold">Voucher No</div><div className="font-black text-[#1D3557] text-lg">{selectedVoucher.voucherNo}</div></div>
            ...
            <table className="w-full text-sm border-collapse">
              ...
              <tbody className="divide-y divide-slate-100">
                {(selectedVoucher.entries || []).map((e: any, i: number) => (
                  <tr key={i} className="hover:bg-blue-50/30">
                    ...
                    <td className="p-2 text-xs font-bold border-r border-slate-100"><span className={e.debit > 0 ? 'text-blue-700' : 'text-red-600'}>{e.debit > 0 ? 'Dr' : 'Cr'}</span></td>
              ...
              <tfoot className="bg-[#1D3557] text-white font-black text-sm">
                <tr>
        ...
        <div className="p-4 bg-slate-50 border-b border-slate-200 shrink-0">
          <div className="grid grid-cols-12 gap-3 items-end">
            <div className="col-span-2">
              <label className="block text-[10px] font-black text-slate-500 uppercase mb-1">Voucher No.</label>
              <input value={voucherNo} onChange={e => setVoucherNo(e.target.value)} className="w-full text-sm font-bold border-b border-slate-300 p-1 bg-yellow-50 outline-none text-red-700 focus:border-blue-500"/>
            </div>
            <div className="col-span-2">
              <label className="block text-[10px] font-black text-slate-500 uppercase mb-1">Voucher Date</label>
              <input type="date" value={date} onChange={e => setDate(e.target.value)} className="w-full text-sm font-bold border-b border-slate-300 p-1 outline-none focus:border-blue-500"/>
            </div>
            <div className="col-span-2">
              <label className="block text-[10px] font-black text-slate-500 uppercase mb-1">Reference No.</label>
              <input value={reference} onChange={e => setReference(e.target.value)} placeholder="Ref / Cheque No." className="w-full text-sm border-b border-slate-300 p-1 outline-none focus:border-blue-500"/>
            </div>
            <div className="col-span-6">
              <label className="block text-[10px] font-black text-slate-500 uppercase mb-1">Narration (Common)</label>
              <input value={narration} onChange={e => setNarration(e.target.value)} placeholder="Being amount paid/received for..." className="w-full text-sm italic border-b border-slate-300 p-1 outline-none focus:border-blue-500 text-slate-600"/>
            </div>
          </div>
        </div>
    ...
    <div className="bg-white rounded-xl border border-slate-200 shadow-sm flex-1 flex flex-col h-full overflow-hidden">
      ...
      <div className="p-4 border-b border-slate-200 bg-slate-50 shrink-0">
        <div className="flex justify-between items-center mb-3">
          <div>
            <h3 className="text-lg font-black text-slate-800 tracking-tight">Journal Vouchers</h3>
      ...
      <div className="flex-1 overflow-auto">
        {loading ? (
          <div className="flex items-center justify-center h-full text-slate-400 text-sm gap-2"><div className="animate-spin w-5 h-5 border-2 border-blue-500 border-t-transparent rounded-full"/><span>Loading vouchers...</span></div>
          ...
          <table className="w-full text-left border-collapse text-sm">
            <thead className="bg-[#E4ECEF] sticky top-0 border-b border-slate-300 shadow-sm text-xs text-slate-600 uppercase tracking-wider z-10">
              <tr>
                ...
                <th className="p-2 font-black border-r border-slate-300 w-36">Voucher No</th>
                <th className="p-2 font-black border-r border-slate-300">Narration</th>
                ...
                <th className="p-2 font-black border-r border-slate-300 text-center w-24">Status</th>
                <th className="p-2 font-black text-center w-28">Actions</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-slate-100">
              {filteredVouchers.map((v, i) => (
                <tr key={i} className="hover:bg-blue-50/40 group">
                  ...
                  <td className="p-2 border-r border-slate-100 font-black text-[#1D3557] cursor-pointer hover:underline" onClick={() => viewVoucher(v)}>{v.voucherNo}</td>
                  ...
                  <td className="p-2 border-r border-slate-100 text-center">
                    <span className={`px-2 py-0.5 text-[10px] font-black uppercase rounded-full border ${STATUS_COLORS[v.status] || 'bg-slate-100 text-slate-500 border-slate-200'}`}>{v.status}</span>
                  </td>
                  <td className="p-2 text-center">
                    <div className="flex items-center justify-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                      <button onClick={() => viewVoucher(v)} title="View" className="p-1 text-blue-600 hover:bg-blue-50 rounded"><Eye size={14}/></button>
                      {v.status === 'Posted' && (
