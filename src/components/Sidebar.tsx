...
const MetapharsicLogo = ({ isOpen }: { isOpen: boolean }) => (
 ...
 <div className="absolute inset-0 bg-accent/20 rounded-full blur-md group-hover:bg-blue-400/30 transition-all duration-700"></div>
 ...
 <span className="text-[8px] text-blue-300 font-bold tracking-widest uppercase mt-0.5 opacity-80">
 LIFESCIENCES
 </span>
 </div>
 )}
...
const Sidebar: React.FC<SidebarProps> = ({ 
 ...
 <div className="absolute inset-0 bg-accent/20 rounded-full blur-sm group-hover:bg-blue-400/30 transition-all duration-700"></div>
 ...
 <span className="text-xs font-bold text-slate-800 tracking-widest uppercase leading-none mb-0.5">Metapharsic</span>
 <span className="text-[8px] text-accent font-bold uppercase tracking-tighter leading-none">Enterprise Hub</span>
 </div>
 )}
 </div>
 </div>
 ...
 <div className="w-7 h-7 flex-shrink-0 rounded bg-slate-100 border border-slate-200 flex items-center justify-center text-[10px] font-bold text-slate-600">
 {user?.name.charAt(0)}
 </div>
 {isOpen && (
